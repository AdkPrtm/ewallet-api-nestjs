...
export class SupabaseService {
  ...
  async uploadImageService(
    ...
    const { fileName, extension, base64Data, MimeType }: ImageData =
      getDataImageBase64(base64);

    try {
      let response: SupabaseResponse;
      const path =
        type === 'NewImage' ? `${fileName}.${extension}` : fileNameUser;

      if (type) {
        response = await uploadToStorage(
          this.supabase,
          path,
          base64Data,
          MimeType,
