...
import { RegisterRequest, AuthResponse, LoginRequest, CheckDataExistsRequest, CheckDataExistsResponse, OTPRequest, VerificationOTPRequest } from '../model/auth.model';
...
export class AuthService {
    ...
    async checkDataExists(request: CheckDataExistsRequest): Promise<CheckDataExistsResponse> {
        const checkDataExistsRequest = this.validationService.validate(AuthValidation.CHECKDATAEXISTS, request);
    ...
    async register(request: RegisterRequest): Promise<AuthResponse> {
        ...
        if (checkAvailUser) throw new HttpException('Account already exists', HttpStatus.BAD_REQUEST)
        ...
        if (registerRequest.profile_picture) {
            uploadProfileImage = await this.supabaseService.uploadImageService(registerRequest.profile_picture, 'NewImage')
        }
        ...
        const randomNumber = generateNumber(12)
    ...
    async login(request: LoginRequest): Promise<AuthResponse> {
        ...
        if (!checkAvailUser) throw new HttpException('User not found', HttpStatus.BAD_REQUEST)
        const validPassword = await bcrypt.compare(loginRequest.password, checkAvailUser.password)
    ...
    async sendOTPRegisterService(request: OTPRequest): Promise<void> {
        ...
        if (!haveRequestBefore) {
            ...
            if (!createRequestOtp) throw new HttpException("Something went wrong", 500)
            this.mailService.sendMailOTPRegister(request.email, otp)

        } else {

            if (!isAfter(Date.now(), addMinutes(haveRequestBefore.createdAt, 1))) throw new HttpException("Waiting for cooling down", 400)
            ...
            if (!updateRequestOtp) throw new HttpException("Something went wrong", 500)
            this.mailService.sendMailOTPRegister(request.email, otp)
        }
    }

    async verificationOTPRegisterService(request: VerificationOTPRequest): Promise<void> {
        ...
        if (isAfter(Date.now(), dataMailRequest.expiresAt)) throw new HttpException("OTP Expire, please resend new otp", 400)

        if (dataMailRequest.OTP !== request.otp) throw new HttpException("OTP not match", 400)
