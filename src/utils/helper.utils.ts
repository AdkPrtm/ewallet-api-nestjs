...
export function generateRandomString(
  length: number,
  alfabet: boolean = false,
): string {
  const chars: string = alfabet
...
export function getDataImageBase64(
  base64: string,
  ...
  const fileName =
    fileNameDefault ?? crypto.randomBytes(16).toString('hex').slice(0, 16);
  return {
    fileName,
    extension,
    base64Data,
